...
<template>
  <div class="prompts-page">
    <header>
      <h1>📝 角色管理</h1>
      <button class="btn btn-primary" @click="openNew">+ 新建角色</button>
    </header>

    <div v-if="showForm" class="form-overlay" @click.self="showForm = false">
      <div class="form-card">
        <h3>{{ editId ? "编辑" : "新建" }}角色</h3>
        <div class="field">
          <label>标题</label>
          <input v-model="formTitle" placeholder="角色名称" />
        </div>
        <div class="field">
          <label>类型</label>
        ...
        <div class="field">
          <label>角色提示词</label>
          <textarea v-model="formContent" rows="8" placeholder="描述该角色的行为、能力、调用工具的规则..." />
        </div>
    ...
    <div class="cat-bar">
      <button
        ...
        @click="activeCat = cat"
      >
        {{ { all: '全部', custom: '自定义', system: '系统', blender: 'Blender', gimp: 'GIMP' }[cat] }}
      </button>
    </div>
      ...
      <table class="prompt-table">
        ...
        <tbody>
          <tr v-for="(p, i) in filtered" :key="p.id">
            ...
            <td class="col-title">
              <input
                v-if="editingCells[p.id]"
                v-model="editingTitles[p.id]"
                class="inline-input"
              />
            ...
            <td class="col-action">
              <template v-if="editingCells[p.id]">
                <button class="btn-xs btn-save" @click="saveEdit(p)">保存</button>
                <button class="btn-xs" @click="cancelEdit(p.id)">取消</button>
              </template>
              <template v-else>
                <button class="btn-xs" @click="startEdit(p)">编辑</button>
                <button class="btn-xs btn-danger" @click="doDelete(p.id)">删除</button>
              </template>
            </td>
          </tr>
          <tr v-if="!filtered.length">
            <td colspan="5" class="empty-cell">暂无角色</td>
          </tr>
        </tbody>
      </table>
    </div>
