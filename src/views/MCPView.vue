...
onMounted(async () => {
  ...
  mcpStore.setConnections(
    Array.isArray((await listMCPConnections()).data) ? (await listMCPConnections()).data : [],
  );
  await fetchLogs();
...
<template>
  <div class="bridge-page">
    ...
    <div class="log-section">
      <div class="log-tabs">
        <button
          :class="['log-tab', { active: activeTab === 'all' }]"
          @click="activeTab = 'all'"
        >全部</button>
        <button
          v-for="c in mcpStore.connections"
          :key="c.id"
          :class="['log-tab', { active: activeTab === c.id }]"
      ...
      <div class="log-toolbar" v-if="currentConn">
        ...
        <span :class="['status-badge', currentConn.status]">
          {{ currentConn.status === "connected" ? "已连接" : "未连接" }}
        </span>
        <button
          v-if="currentConn.status === 'disconnected'"
          :disabled="connecting === currentConn.id"
          class="btn btn-connect"
          @click="doConnect(currentConn.id)"
        >
          {{ connecting === currentConn.id ? "连接中..." : "连接" }}
        </button>
        <button
          v-if="currentConn.status === 'connected'"
          class="btn btn-disconnect"
          @click="doDisconnect(currentConn.id)"
        >
          断开
        </button>
      ...
      <div class="log-toolbar" v-else>
        <span class="conn-name">所有服务</span>
      </div>
      <div class="log-area">
        <div v-if="displayLogs.length === 0" class="log-empty">暂无日志（每 3 秒自动刷新）</div>
